 ...
 * 
 *         calendars are retrieved in two ways - by path (with get methods) - by custom query (with
...
public class CalDAVCollection extends CalDAVCalendarCollectionBase {
     ...
     */
    public CalDAVCollection(String path, HostConfiguration hostConfiguration,
    ...
    /**
     * add a standalone recurrent event in a recurrent evet series, so that this standalone event's
     * propertis can be modified independently
     * 
     * @param httpClient
     * @param vevent
     * @param uid
     ...
     * 
     */
    public void generatChildRecurrentEvent(HttpClient httpClient, String uid, int instanceNumber, Date start,
            Date end) throws CalDAV4JException {
        ...
        try {
            ...
            for (int i = 0; i < eventNumberToBeCreated; i++) {
                ...
                if (notCreated(uid, period, calendar)) {
                    generation++;
     ...
     * 
     * Tries to use the event UID followed by ".ics" as the name of the resource, otherwise will use
     * the UID followed by a random number and ".ics"
     * 
     * @param httpClient the httpClient which will make the request
        ...
        for (int x = 0; x < 3 && !didIt; x++) {
            ...
            try {
                ...
                String etag = StringUtils.defaultString(UrlUtils.getHeaderPrettyValue(putMethod, HEADER_ETAG),
                        "");
                CalDAVResource calDAVResource = new CalDAVResource(calendar, etag,
                    getHref((putMethod.getPath())));
     ...
     * given VEvent
     * 
     * TODO: Deal with SEQUENCE TODO: Handle timezone!!! Right now ignoring the param...
     *
     * @param httpClient the httpClient which will make the request
    ...
    public void updateRecurrentStandaloneEvent(HttpClient httpClient, VEvent vevent, VTimeZone timezone)
            throws CalDAV4JException, IOException, URISyntaxException, ParseException {
        String uid = getUIDValue(vevent);
        ...
        occurrence.getProperty(Property.DTSTART)
                .setValue(ICalendarUtils.getPropertyValue(vevent, Property.DTSTART));
        occurrence.getProperty(Property.DTEND)
                .setValue(ICalendarUtils.getPropertyValue(vevent, Property.DTEND));
        occurrence.getProperty(Property.DESCRIPTION)
                .setValue(ICalendarUtils.getPropertyValue(vevent, Property.DESCRIPTION));
        occurrence.getProperty(Property.SUMMARY)
                .setValue(ICalendarUtils.getPropertyValue(vevent, Property.SUMMARY));
     ...
     * 
     * @param httpClient the httpClient which will make the request
     * @param relativePath the path, relative to the collection path for which to grant the ticket
     *            on
     * @param visits
     * @param timeout
     * @param read
     ...
     * @throws CalDAV4JException Is thrown if the execution of the MkTicketMethod fails
     */
    public String createTicket(HttpClient httpClient, String relativePath, Integer visits, Integer timeout,
        ...
        try {
            ...
            if (statusCode != CaldavStatus.SC_OK) {
                throw new CalDAV4JException("Create Ticket Failed with Status: " + statusCode +
     ...
     * 
     * @param httpClient the httpClient which will make the request
     * @param relativePath the path, relative to the collection path for which to revoke the ticket
     * @param ticketID the ticketID which to revoke
     * @throws CalDAV4JException Is thrown if the execution of the DelTicketMethod fails
     */
    ...
    /**
     * Returns all the ticket ID's from all tickets the requesting user has permision to view on a
     * resource.
     * 
     * @param httpClient the httpClient which will make the request
     * @param relativePath the path, relative to the collection path for which to get the tickets
     * @return
     * @throws CalDAV4JException
     ...
     * @param path
     * @return
     * @throws CalDAV4JException FIXME testme
     */
    ...
    protected CalDAVResource getCalDAVResourceFromServer(HttpClient httpClient, String path)
        ...
        try {
            ...
            throw new CalDAV4JException(
                "Malformed calendar resource returned at path: " + getMethod.getPath(), e);
        }
    ...
    private boolean isGoogleTombstone(Calendar calendar) {
        /*
         * if (this.skipGoogleTombstones && (calendar != null )) { // is it a tombstone? if (
         * calendar.getComponents().size() ==1 && (calendar.getProductId().getValue().matches(
         * "Google Calendar")) && (calendar.getComponents().get(0) instanceof VTimeZone ) ) return
         * true; }
         */
        return false;
    }
     ...
     */
    protected List<String> getComponentProperty(HttpClient httpClient, String componentName,
    ...
    /**
     * return a list of components using REPORT without passing thru CaldavResource
     * 
     * @param query
     * @return
     * @throws CalDAV4JException
     ...
     * 
     * The use of caching changes the behavior of this method. if cache is not enable, returns a
    ...
    protected List<CalDAVResource> getCalDAVResources(HttpClient httpClient, CalendarQuery query)
        ...
        try {
            ...
            throw new CalDAV4JException("Can't connecto to " + getHostConfiguration().getHostURL(),
                connEx.getCause());
        } catch (Exception he) {
            throw new CalDAV4JException("Problem executing method", he);
        }
        ...
        while (responseEnum.hasMoreElements()) {
            try {
                ...
                if (usingCache) {
                    CalDAVResource resource = getCalDAVResource(httpClient, stripHost(response.getHref()),
                            etag);
                    list.add(resource);
                    cache.putResource(resource);
                    /*
                     * dead code, to be reenabled in case of tombstones // avoid parsing object if
                     * not required if (isSkipGoogleTombstones()) { list.add(resource);
                     * cache.putResource(resource); } else if (!
                     * isGoogleTombstone(resource.getCalendar())){ list.add(resource);
                     * cache.putResource(resource); }
                     */
                } else {
                    if (response != null) {
    ...
    public int testConnection(HttpClient httpClient) throws CalDAV4JException {
        ...
        switch (method.getStatusCode()) {
            case CaldavStatus.SC_OK:
                break;
            default:
                throw new BadStatusException(method.getStatusCode(), method.getName(),
    ...
    /*
     * // if set to true, caldav4j will skip timezone-only calendars // this should be set to false
     * to find deleted events private boolean skipGoogleTombstones = false;
     * 
     * public boolean isSkipGoogleTombstones() { return skipGoogleTombstones; }
     * 
     * public void setSkipGoogleTombstones(boolean skipGoogleTombstones) { this.skipGoogleTombstones
     * = skipGoogleTombstones; }
     */
